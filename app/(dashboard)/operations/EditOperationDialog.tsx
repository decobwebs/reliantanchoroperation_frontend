...
/** Section heading, matching Create's typography so the two dialogs read alike. */
 ...
 *
 * Laid out as its own dialog rather than through ReasonGatedDialog — that one is
 * a small (max-w-sm) "type a reason and confirm" shell, and a form this size
 * overflowed it in both directions. The shape here mirrors CreateOperationDialog
 * (max-w-2xl, one section per group) so Edit and Create read as the same form.
 * Reason and actions sit in a fixed footer, so Save stays reachable instead of
 * living below a long scroll.
 *
 ...
 *    has no route out of. The backend refuses it too — this just does not offer
 ...
 *    same way (one call each after the operation exists). They sit together in
 *    one section, flagged once, instead of repeating the caveat per block.
 */
export function EditOperationDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* Header and footer are fixed; only the middle scrolls, and it scrolls
          vertically only — every column inside is min-w-0 so nothing can force
          the dialog sideways the way the old max-w-sm shell did. */}
      <DialogContent
        className="flex max-h-[92vh] w-[calc(100vw-1.5rem)] flex-col gap-0 overflow-x-hidden overflow-y-hidden p-0 sm:max-w-2xl"
      >
        <DialogHeader className="shrink-0 border-b px-5 py-4 sm:px-6">
          <DialogTitle className="flex items-center gap-2 text-[15px] font-bold tracking-tight">
            <Pencil className="h-4 w-4 text-brand-600" strokeWidth={2.2} />
            Edit {operation.operation_number}
          </DialogTitle>
          <DialogDescription className="text-[12px]">
            Every field from Create, editable at any phase — changes are logged with your reason.
          </DialogDescription>
        </DialogHeader>

        <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-5 py-5 sm:px-6">

          {/* ── Operation details — held until Save ───────────────────── */}
          <div className="space-y-3">
              ...
              <div className="min-w-0 space-y-1.5">
                ...
                {!isDraft && (
                  <p className="text-[11px] text-muted-foreground">
                    Locked once past Draft — type decides which pipeline this follows.
                  </p>
                )}
              </div>
              <div className="min-w-0 space-y-1.5">
            ...
            <div className="min-w-0 space-y-1.5">
              ...
              <Textarea rows={2} className="resize-none text-sm" value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
          </div>

          {/* ── Products — held until Save, replaced wholesale ────────── */}
          <div className="space-y-3 border-t pt-5">
            <div className="flex items-start justify-between gap-3">
              <SectionTitle hint="At least one is required — this sets the expected volume.">Products</SectionTitle>
              <Button
                type="button" size="sm" variant="outline" className="h-7 shrink-0 gap-1 text-[11px]"
            ...
            {products.length === 0 ? (
              <div className="rounded-lg border border-dashed px-4 py-5 text-center text-xs text-muted-foreground">
                No products — add at least one before saving.
              </div>
            ) : (
                ...
                {products.map((p, i) => (
                  <div key={i} className="flex items-end gap-2">
                    <div className="min-w-0 flex-1 space-y-1">
                      {i === 0 && <Label className="text-[10px] text-muted-foreground">Product Type</Label>}
                      <Select
                    ...
                    <div className="w-24 shrink-0 space-y-1 sm:w-32">
                      {i === 0 && <Label className="text-[10px] text-muted-foreground">Quantity (MT)</Label>}
                      <Input
                        type="number" step="0.001" min="0" className="h-9 text-xs"
                        value={p.quantity_mt}
          ...
          {/* ── Linked records — each applies on the spot, not on Save ── */}
          <div className="space-y-4 border-t pt-5">
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 dark:border-amber-500/25 dark:bg-amber-500/10">
              <Zap className="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-600 dark:text-amber-400" strokeWidth={2.2} />
              <p className="text-[11px] leading-relaxed text-amber-900 dark:text-amber-200">
                <span className="font-semibold">Everything below saves as you change it.</span>{" "}
                These are separate records with their own audit trails — adding or removing one
                takes effect straight away, whether or not you press Save.
              </p>
            </div>

            {needsVessel && (
              <div className="space-y-2">
                <SectionTitle>Naval Clearances</SectionTitle>
                {(operation.naval_clearances ?? []).length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {operation.naval_clearances?.map((nc) => (
                      <span key={nc.id} className="inline-flex items-center gap-1.5 rounded-full border bg-muted/40 px-2.5 py-1 text-[11px]">
                        {nc.clearance_number}
                        <button
                          type="button"
                          aria-label={`Unlink ${nc.clearance_number}`}
                ...
                <Select value={addNcId} onValueChange={(v) => { setAddNcId(v); linkNc.mutate(v); }}>
                  <SelectTrigger className="h-9 text-xs"><SelectValue placeholder="Add a Naval Clearance…" /></SelectTrigger>
                  <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-[1fr_1fr_7rem_auto]">
                ...
                <div className="flex items-end">
                  <Button
                    type="button" size="sm" className="h-9 w-full text-xs"
                    disabled={!newAssignee || !newTaskType || addTask.isPending}
                    onClick={() => addTask.mutate()}
                  >
                    {addTask.isPending ? <Spinner size={14} /> : "Add"}
                  </Button>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <SectionTitle>PFI Allocations</SectionTitle>
              {allocations?.length ? (
                <div className="space-y-1">
                  {allocations.map((a) => (
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-[1fr_9rem_auto]">
                <div className="min-w-0 space-y-1">
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  <Input
                    type="number" step="0.01" min="0" className="h-9 text-xs"
                ...
                <div className="flex items-end">
                  ...
                  >
                    {addAllocation.isPending ? <Spinner size={14} /> : "Add"}
                  </Button>
                </div>
              </div>
            </div>
        ...
        {/* Reason and actions stay put, so Save never sits below a long form. */}
        <div className="shrink-0 space-y-3 border-t bg-muted/30 px-5 py-4 sm:px-6">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              disabled={!reason.trim() || products.length === 0 || mutation.isPending}
